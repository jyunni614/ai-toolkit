...
class SDTrainer(BaseSDTrainProcess):
    ...
    def hook_before_train_loop(self):
        ...
        zimage_preunloaded_te = (
            self.model_config.arch == "zimage"
            and self.is_caching_text_embeddings
            and hasattr(self.sd, "_has_real_text_encoder")
            and not self.sd._has_real_text_encoder()
        )
        ...
        if self.train_config.unload_text_encoder or self.is_caching_text_embeddings:
            if zimage_preunloaded_te:
                print_acc("Text embeddings already cached and text encoder already unloaded; skipping recache/unload in hook_before_train_loop")
            else:
